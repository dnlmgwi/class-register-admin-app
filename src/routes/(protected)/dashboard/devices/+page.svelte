<script lang="ts">
  ...
  const {
    ...
    validateForm: validateDeviceForm,
    enhance: deviceEnhance,
  } = superForm(data.deviceForm, {
    validators: zod(editDeviceSchema),
    onResult: ({ result }) => {
      if (result.type === "success") {
        ...
        toast.error(result.data?.error || "Failed to edit device");
      }
    },
  });
  ...
  const editDialog = editDialogStore();
...
<Dialog.Root bind:open={$tokenDialogOpen}>
  <Dialog.Content class="sm:max-w-[425px]">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Device token successfully created! Please save the token below, as it
  ...
  <Dialog.Root bind:open={$createDialogOpen}>
    ...
    <Dialog.Content class="sm:max-w-[425px]">
      <Dialog.Header>
        ...
        <Dialog.Description>
          The device name should uniquely indicate the room or location where
            ...
            >
              ...
              >
                <div class="flex items-center justify-end gap-2">
                  <AlertDialog.Root
                    open={$editDialog.get(device.id)?.action === "edit" &&
                      $editDialog.get(device.id)?.index === index}
                  >
                    <AlertDialog.Trigger
                      class="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 rounded-md px-3"
                      on:click={() => {
                        editDialog.open(device.id, "Edit", index);
                    ...
                    <AlertDialog.Content>
                      <AlertDialog.Header>
                        <AlertDialog.Title>Edit Device</AlertDialog.Title>
                        <AlertDialog.Description>
                          Editing this device will require you to update its
                          access token.
                        </AlertDialog.Description>
                      </AlertDialog.Header>
                      <form method="POST" action="?/edit" use:deviceEnhance>
                        <input
                          type="hidden"
                          name="id"
                          id="id"
                          bind:value={$deviceForm.id}
                        />
                        <div class="grid gap-4 py-4">
                          <div class="grid grid-cols-4 items-center gap-4">
                            <Label for="name" class="text-right">Name</Label>
                        ...
                        <AlertDialog.Footer>
                          <AlertDialog.Cancel
                            onclick={() => editDialog.close(device.id)}
                          >
                            Cancel
                          </AlertDialog.Cancel>
                  ...
                  {#if device.status === "offline"}
                    <AlertDialog.Root
                      open={$dialogStore.get(device.id)?.action === "enable" &&
                        $dialogStore.get(device.id)?.index === index}
                    >
                      <AlertDialog.Trigger
                        class="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 rounded-md px-3"
                        on:click={() =>
                          dialogStore.open(device.id, "enable", index)}
                      >
                        Enable
                      </AlertDialog.Trigger>
                      <AlertDialog.Content>
                        ...
                        >
                          ...
                          <AlertDialog.Footer>
                            <AlertDialog.Cancel
                              on:click={() => dialogStore.close(device.id)}
                            >
                              Cancel
                            </AlertDialog.Cancel>
                            <AlertDialog.Action type="submit"
                  ...
                  {:else}
                    ...
                    >
                      <AlertDialog.Trigger
                        class="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 rounded-md px-3"
                        on:click={() =>
                          dialogStore.open(device.id, "disable", index)}
                      >
                        Disable
                      </AlertDialog.Trigger>
                      <AlertDialog.Content>
                  ...
                  >
                    <AlertDialog.Trigger
                      class="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 rounded-md px-3"
                      on:click={() =>
                        dialogStore.open(device.id, "delete", index)}
                    >
                      Delete
                    </AlertDialog.Trigger>
                    <AlertDialog.Content>
